...
namespace ZucchiArchive\Adapter;
...
class Zip extends AbstractAlgorithm
{
    ...
    public function getArchive()
    {
        ...
        if (empty($archive) || !file_exists($archive)) {
            throw new Exception\RuntimeException(sprintf('Zip Archive "%s" not found', $archive));
        }
    ...
    {
        $archive = $this->getArchive();
        $data = $archive->getFromName($file);
        if (!$data) {
            throw new Exception\RuntimeException(sprintf('Unable to retrieve the contents of "%s"', $file));
        }
        return $data;
    }
